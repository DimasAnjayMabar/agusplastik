...
class _DistributorViewState extends State<DistributorView> {
  ...
  Future<Map<String, dynamic>> fetchDistributorDetails(
    ...
    try {
      final user = await StorageService.getDatabaseIdentity();
      final pass = await StorageService.getPassword();

      final response = await http.post(
        Uri.parse('http://${user['serverIp']}:3000/distributor-details'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
          'servername': user['serverIp'],
          'username': user['serverUsername'],
          'password': pass,
          'database': user['serverDatabase'],
  ...
  Future<void> _showDistributorDetails(
      BuildContext context, distributorId) async {
    try {
